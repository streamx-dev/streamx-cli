...
function check_http_response {
    ...
    Write-Output "Checking '$URL' response"

    if ($RESPONSE_STATUS -eq $HTTP_CODE) {
        if ($RESPONSE.Content -match $CONTENT) {
            Write-Output "Published as expected"
        } else {
...
function check_binary_file {
    param(
        [string]$URL,
        [string]$IMAGE_PATH
    )

    Invoke-WebRequest -Uri $URL -OutFile "C:\temp\downloaded_image.jpg"
    Write-Output "Checking '$URL' file"

    if (Compare-Object (Get-Content "C:\temp\downloaded_image.jpg" -Raw) (Get-Content $IMAGE_PATH -Raw)) {
